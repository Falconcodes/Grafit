...
#define LAMP PORTB.3 //лампа на корпусе - горит когда колпак одет (не забываем править DDRx)
...
void main(void)
{#asm ("sei");     
 DDRB.3=DDRB.4=DDRC.0=DDRD.0=DDRD.7=1; //выходы
...
//До подачи сигнала Старт крутимся в этом цикле, моргаем светодиодом редко и ждем сигнала. Флаг ожидания - wait
 ...
 // FINISH=0; //если перед этим циклом был еще один, сбрасываем сработавшее реле финиша
...
///////////////////////////////
 ...
 void pwm(int delay_min, int pause) {   
    ...
    if (delay==delay_on) {  //после определенного количества переполнений таймера (= паузы после начала полупериода), включаем тиристор (ниже)
      ...
      delay_us(50); //задержка на высоком уровне
