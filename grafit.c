...
//Параметры 1 этапа
#define MIN_PWM_1 10 //процент от максимально допустимой мощности при данном включении, с которого начнет увеличиваться мощность
#define MAX_PWM_1 17 //процент от максимальной мощности, до которого увеличится мощность на этапе 1 (прогрев)
...
interrupt [EXT_INT0] void ext_int0_isr(void) {       //Прерывание по началу полупериода сети
...
{#asm ("sei");     
 ...
 PORTB.7=1;
 //LOCK_OFF=1; //подтяжка к Vcc
 
 LED=0; //включаем светодиод на 1 сек при перезагрузке или первом включении
 delay_ms(1000);
 LED=1;
 ...
 AGAIN: ; //попадаем сюда, если колпак был снят в течение нагрева и одет обратно, и/или сброшен внешний сигнал СТАРТ  
  while (again) { //цикл ожидания сброса внешнего сигнала Старт
    while(START); //если сигнал СТАРТ все еще подан, ждем, пока будет обнулен чтобы продожить
    delay_ms(50); //ждем 50мсек для исключения помех и дребезга и проверяем повторно, что внешний сигнал Старт сброшен
    if (!START) again=0;  // сбрасываем флаг возврата, чтобы выйти из этого цикла 
  }
  
  
 //Крутимся в этом цикле перед запуском плавки, ждем соблюдения условий: колпак не открыт и подан сигнал Старт
  while(wait) {
  ...
  //далее идет два цикла, обеспечивающие ожидание сигнала START прим закрытом колпаке, и при этом медленное краткое мигание светодиода
  LED=0; //зажигаем светодиод на плате
   ...
   LED=1; //гасим светодиод на плате
   for (i=1;i<10000; i++) {
   delay_us(50);
   if  (START&(!KOLPAK)) wait=0;  // Если дан сигнал старта при неоткрытом колпаке, флаг ожидания сбрасываем для выхода из цикла ожидания
   if (KOLPAK) LAMP=0;
 ...
 // LOCK_OFF=1; //обесточиваем замок дверцы
    ...
    LAMP=0; //коплак открыли - гасим лампу ОК
    again=1; //флаг ожидания СБРОСА старого сигнала Старт установлен (используется в цикле выше после while(1); )
    wait=1; // флаг ожидания нового сигнала Старт и одетого колпака - снова установлен
    goto AGAIN; // если колпак сняли, идем в начало, где ожидаем одевания колпака и сигнала "старт".
   }

 //ПЕРВЫЙ ПЕРИОД - ОБЖИГ ПАРАФИНА
 delay_on=pwm_to_delay(MIN_PWM_1);
 busy=-1;  // -1 чтобы при декременте не стал 0, как во втором периоде, где busy=1. декремент делается, если графит перегорел. На первом этапе это не происходит, спасаем от глюков с неконтактом.
 ...
 GICR|=0b01000000;     //разрешаем внешние прерывания через Int0   
   ...
   delay_ms(1000);
   FINISH=0;  
// delay_ms(60000); //в версии с дверцей - открываем замок после минуты ожидания - чтобы крокодилы могли остыть
// LOCK_OFF=1; 
...
///////////////////////////////
 ...
 void pwm(int delay_min, int pause) {   
    ...
    if (delay==delay_on) {  //после определенного количества переполнений таймера (= паузы после начала полупериода), включаем тиристор (ниже)
      ...
      if (busy==-120)  {   //чтобы не случалось переполнение переменной при декрементах 
        busy=-1;
        }
      }
   ...
   if (KOLPAK || (!START)) {    //делаем проверку, если КОЛПАК открыт ИЛИ внешний сигнал СТАРТ сброшен, то уходим в бесконечный цикл пока не закроют колпак
         GICR=0;  //запрещаем внешнее прерывание
         TIMSK=0; //запрет прерывания по ТС0
         TCNT0=0; //обнуляем счетный регистр
         TIR_OUT=0;
         LAMP=0; //гаснет лампа на корпусе
         delay_ms(10); //ждем один полупериод для закрытия тиристора и гарантии прекращения тока через реле
         VOLTAGE=0; //и отключае реле
    while(KOLPAK){
           LED=0;
           delay_ms(50);
           LED=1;
     ...
     wait=1; //когда вернемся в цикл ожидания СТАРТа и установленного колпака, этот флаг скажет что ожидание необходимо
     delay_ms(200);
   } 
 }
